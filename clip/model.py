...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'IVLP' or current_trainer == 'VPT':
            ...
            assert current_trainer == 'CoOp' or current_trainer == 'CoCoOp' or current_trainer == 'CLIP'
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor, require_internal=False):
        BB, CC, HH, WW = x.shape 
        ...
        _, FF, _ = x.shape  
        if FF != self.positional_embedding.shape[0]: 
            pos_embed = resample_abs_pos_embed(
                self.positional_embedding.to(x.dtype).unsqueeze(dim=0),
                (HH//16, WW//16)
            ) 
        else:
            pos_embed = self.positional_embedding.to(x.dtype).unsqueeze(dim=0) 
        x = x + pos_embed 
...
class VisionTransformer_MaPLe(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts, require_internal=False):
        BB, CC, HH, WW = x.shape 
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text, need_ema=True): 
    ...
    def fafa(self, prompts, tokenized_prompts): 
