...
class VisionTransformer(nn.Module):
    ...
    def get_extra(self, patchemb, prompts, projs, mods):
        # B N D 
        if projs is not None:
            prompts = projs(prompts)  
            ssstack = patchemb.mean(dim=1,keepdim=True)
        else:
            ssstack = []
            for item in mods:
                ssstack.append((patchemb*item).mean(dim=1,keepdim=True)) 
            ssstack = torch.cat(ssstack,dim=1) 

            
        attn = torch.einsum("bjd,nd->bjn",ssstack,prompts).softmax(dim=-1)
        rest = torch.einsum("bjn,nd->bjd",attn,prompts)
        return rest 
