...
from kdclip import clip
from kdclip.simple_tokenizer import SimpleTokenizer as _Tokenizer 
...
class Feature_Trans_Module_two_layer(nn.Module):
    def __init__(self, input_dim=100, out_dim=256):
        ...
        self.vprompts = nn.Parameter(torch.randn((20, 768),dtype=torch.half).cuda()) 
        self.endprompts = nn.Parameter(torch.randn((20, 512),dtype=torch.half).cuda()) 
        self.mods = nn.ParameterList([nn.Parameter(torch.randn((1,1,768),dtype=torch.half).cuda())  for _ in range(4)])
        self.promid = nn.Sequential(
            nn.Linear(768,128),
            nn.ReLU(),
            nn.Linear(128,1024)
        ).cuda()
...
class CustomCLIP(nn.Module):
    ...
    def get_final(self, patchemb, prompts):
        # B N D  
        attn = torch.einsum("nd,bmd->bnm",prompts, patchemb).softmax(dim=-1)
        rest = torch.einsum("bnm,bmd->bnd",attn, patchemb)
        return rest
...
class REDIS(TrainerX):
    ...
    def forward_backward(self, batch):
        ...
        image_ft, logit_scale = model(niimage, label) 
