...
class DatasetWrapper(TorchDataset):
    ...
    def __init__(self, cfg, data_source, transform=None, is_train=False):
        ...
        if isinstance(transform, list):
            self.transform = transform[0]  # accept list (tuple) as input
            self.niform = transform[-1]
        else: 
    ...
    def __getitem__(self, idx):
        ...
        # idea constrast
        if self.niform is not None:
            if isinstance(self.niform, (list, tuple)):
                for i, tfm in enumerate(self.niform):
                    img = self._transform_image(tfm, img0)
                    keyname = "niimg"
                    if (i + 1) > 1:
                        keyname += str(i + 1)
                ...
                img = self._transform_image(self.niform, img0)
                output["niimg"] = img
        else:
            output["niimg"] = img0
        output["nipa"] = item.impath
        # idea constrast
