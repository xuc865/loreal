...
from .corruption import CorruptionTransform, RandomCorruptionTransform
...
def build_transform(cfg, is_train=True, choices=None):
    ...
    target_size = f"{cfg.INPUT.SIZE}x{cfg.INPUT.SIZE}"
...
def _build_transform_train(cfg, choices, target_size, normalize, give_res=0):
    ...
    input_size = cfg.INPUT.SIZE if give_res ==0 else give_res
...
def _build_transform_test(cfg, choices, target_size, normalize, give_res=0):
    ...
    print(f"+ resize the smaller edge to {input_size}")
