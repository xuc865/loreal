...
class TrainerBase:
    ...
    def train(self, start_epoch, max_epoch):
        ...
        self.before_train() 
        if self.cfg.TRAINER.NAME == "ZeroshotCLIP":
            self.after_train()
            return
...
class SimpleTrainer(TrainerBase):
    ...
    def test(self, split=None):
        ...
        for batch_idx, batch in enumerate(tqdm(data_loader)):  
            ...
            output = self.model_inference(nis) 
            print_stats("after model.to")    
...
def print_stats(tag=""):
    ...
    print(f"[{tag}] max_alloc={torch.cuda.max_memory_allocated(device)/1024**2:.1f}MB "
          ...
          f"cur_alloc={torch.cuda.memory_allocated(device)/1024**2:.1f}MB "
          f"cur_reserved={torch.cuda.memory_reserved(device)/1024**2:.1f}MB")